...
namespace Irony.Parsing
{
    ...
    public class Scanner
    {
        ...
        private void NextToken(bool treatCommentHasToken=false)
        {
            // AM
            // Un commentaire n'est plus un token mais devient un trivia
            // On considère ici que :
            //  - Un commentaire ne sera jamais bufferrisé (ds Context.BufferedTokens)
            //  - Que l'appel de ScanToken ne met pas à jour PreviousToken  (sinon on faut le sauvegarder)
            while(true)
            {
                ...
                //2. Skip whitespace.
                _grammar.SkipWhitespace(Context);
                ...
                if (treatCommentHasToken || Context.CurrentToken.Category != TokenCategory.Comment)
                {
                    break;
                }

                Context.Trivias.Enqueue(new CommentTrivia(Context.CurrentToken));
            }

            // Si il y a un previoustoken, tous les trivias qui sont sur sa ligne lui sont rattachés comme trivias de fin
            if (Context.Trivias.Count > 0)
            {
                if (Context.PreviousToken != null)
                {
                    Context.PreviousToken.TrailTrivias = Context.Trivias.DequeueAll(t => !(t is EndOfLineTrivia)); // Tous les trivias jusqu'au 1er saut de ligne rencontré
                }

                // Le reste concerne les trivias de début
                Context.CurrentToken.LeadTrivias = Context.Trivias.DequeueAll();
            }
        ...
        public void VsSetSource(string text, int line)
        {
//            var line = Context.Source == null ? 0 : Context.Source.Location.Line;
            var newLoc = new SourceLocation(0, line + 1, 0);
