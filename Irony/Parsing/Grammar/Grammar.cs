...
namespace Irony.Parsing
{
    ...
    public class Grammar
    {
        ...
        public virtual void SkipWhitespace(ParsingContext context)
        {
            var source = context.Source;

            SourceLocation ws = SourceLocation.Empty;
            bool ws_flag = false;
            ...
            while (!source.EOF())
            {
                switch (source.PreviewChar)
                {
                    case ' ':
                    case '\t':
                        if (!ws_flag) { ws_flag = true; ws = source.Location; }
                        break;
                    case '\n':
                    case '\v':
                    case '\r':
                        if (ws_flag)
                        {
                            context.Trivias.Enqueue(new WhitespaceTrivia(new SourceSpan(ws, source.PreviewPosition - ws.Position)));
                            ws_flag = false;
                        }
                        
                        if (UsesNewLine) return; //do not treat as whitespace if language is line-based
                        if( source.PreviewChar == '\r')
                            context.Trivias.Enqueue(new EndOfLineTrivia(new SourceSpan(source.Location, 2))); // On considère qu'il y aura tjs /r/n
                        break;
                    default:
                        if (ws_flag)
                        {
