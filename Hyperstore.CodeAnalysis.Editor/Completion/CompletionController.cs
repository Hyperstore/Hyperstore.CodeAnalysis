...
namespace Hyperstore.CodeAnalysis.Editor.Completion
{
    ...
    internal class CompletionController : IIntellisenseController, IOleCommandTarget //, IVsExpansionClient 
    {
        ...
        internal CompletionController(IList<ITextBuffer> subjectBuffers, ITextView textView, ICompletionBroker completionBrokerMap)
        {
        ...
        private void ShowCompletion(char currentKey)
        {
            ...
            if (activeSession == null || activeSession.IsDismissed)
            {
                ...
                if (this.textView.Selection.Mode != TextSelectionMode.Box)
                {
                    ...
                    {
                        caretPoint = textView.Caret.Position.Point.GetPoint(textBuffer => (subjectBuffers.Contains(textBuffer)), PositionAffinity.Predecessor);
                    }
                    ...
                    if (caretPoint.HasValue)
                    {
                        ...
                        activeSession = completionBrokerMap.CreateCompletionSession(textView, bufferPosition.Snapshot.CreateTrackingPoint(bufferPosition, PointTrackingMode.Negative), true);
